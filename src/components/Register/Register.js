  ...
  return(
    <>
      <Form name='registerForm' title='Добро пожаловать!' underFormQuestion='Уже зарегистрированы?' linkPath='/signin' linkName='Войти' onSubmit={handleSubmit} >
        <FormInput required={true} minLength={2} maxLenght={30} inputType='text' inputName='name' labelName='Имя' inputId='name' pattern={namePattern} labelFor='name' tabIndex='1' placeholder='' spanText={errors.name} value={values.name || ''} onChange={handleChange} />
        <FormInput required={true} inputType='email' pattern={emailPattern} inputName='email' labelName='E-mail' inputId='email' labelFor='email' tabIndex='2' placeholder='' spanText={errors.email} value={values.email || ''} onChange={handleChange} />
