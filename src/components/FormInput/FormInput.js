...
const FormInput = ({ labelClassName, inputClassName, labelName, inputType, inputName, spanText, tabIndex, value, placeholder, children, labelFor, inputId, onChange }) => {
  return(
    <>
      ...
      <input id={inputId} type={inputType} name={inputName} className={`form__input form__input${inputClassName}`} required tabIndex={tabIndex} value={value} placeholder={placeholder} onChange={onChange} />
